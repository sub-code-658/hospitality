  ...
  return (
    <div className="glass-card p-5 h-full flex flex-col group relative overflow-hidden transition-all duration-300 hover:scale-[1.02]">
      <div className="flex items-start justify-between mb-4">
        ...
        {worker.isAvailable && (
          <span className="flex items-center justify-center whitespace-nowrap px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest bg-green-500/10 text-green-400 border border-green-500/20">{t('common.available', 'Available')}</span>
        )}
      </div>

      <div className="mb-4">
        <p className="text-[var(--text-dim)] text-xs mb-2">{t('common.skills', 'Skills')}</p>
        <div className="flex flex-wrap gap-2">
          ...
          {(!worker.skills || worker.skills.length === 0) && (
            <span className="text-[var(--text-dim)] text-xs italic">{t('common.no_skills_listed', 'No skills listed')}</span>
          )}
        </div>
      </div>
      ...
      <div className="mt-auto flex gap-3">
        ...
        >{t('common.view_profile', 'View Profile')}</Link>
        <button
          onClick={onInvite}
