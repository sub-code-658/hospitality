...
export default function EventCard({ event, showOrganizer = false }) {
  ...
  const totalPay =
    event.rolesNeeded?.reduce((sum, r) => sum + r.payAmount * r.count, 0) || 0;
        ...
        <div className="flex flex-wrap gap-1.5 mb-5">
          ...
          {event.rolesNeeded?.length > 3 && (
            <span
              className="tag"
              style={{
                background: "rgba(237,232,224,0.05)",
                borderColor: "rgba(237,232,224,0.1)",
                color: "var(--text-dim)",
              }}
            >
              +{event.rolesNeeded.length - 3}
