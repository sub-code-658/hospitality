...
export default function PostEventPage() {
  ...
  const validate = () => {
    ...
    roles.forEach((role, idx) => {
      if (role.roleSelect === "Other" && !role.customRole.trim())
        newErrors[`role_${idx}`] = "Custom role name is required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast(
        error.response?.data?.message || "Failed to post event",
        "error",
      );
    } finally {
      setLoading(false);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      <h1 className="text-3xl md:text-4xl font-bold text-[color:var(--text-main)] text-[color:var(--text-primary)] mb-10 animate-slide-up">
        {t("common.post_new_event", "Post New Event")}
      </h1>
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm text-[color:var(--text-main)]/40 text-[color:var(--text-primary)]/40 mt-2">
            {t(
              "common.click_on_the_map_to_set_the_ev",
        ...
        >
          {loading ? (
            ...
            t("common.post_event", "Post Event")
          )}
        </button>
      </form>
    </div>
