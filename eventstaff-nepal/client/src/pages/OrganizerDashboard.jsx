    ...
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        ...
        <div className="card p-6 animate-fade-in stagger-3">
          ...
          <div className="text-sm" style={{ color: 'rgba(180, 150, 100, 0.6)' }}>Pending Applications</div>
        </div>
      </div>
        ...
        <div className="card p-6 lg:col-span-2 animate-fade-in stagger-5">
          ...
          {filteredEvents.length === 0 ? (
            ...
            <div className="space-y-6">
              ...
              {/* Calendar Grid */}
              <div className="grid grid-cols-7 gap-1 text-center text-xs" style={{ color: 'rgba(180, 150, 100, 0.5)' }}>
                  ...
                  return (
                    <button
                      ...
                      className={`min-h-[80px] p-2 rounded-xl text-left border transition-all duration-200 ${
                        isSelected ? 'border-[#b8956a] bg-[rgba(180, 150, 100, 0.1)]' : 'border-[rgba(180, 150, 100, 0.1)]'
                    ...
                    >
                      ...
                      <div className="mt-1.5 space-y-1">
                        {dayEvents.slice(0, 2).map(event => (
                          <div key={event._id} className="rounded px-1.5 py-0.5 text-[10px] truncate" style={{ background: 'rgba(180, 150, 100, 0.15)', color: 'rgba(180, 150, 100, 0.8)' }}>
                            {event.title}
                          </div>
                        ))}
                        {dayEvents.length > 2 && (
                          <div className="text-[10px]" style={{ color: 'rgba(180, 150, 100, 0.4)' }}>+{dayEvents.length - 2}</div>
                        )}
                      </div>
                    </button>
                  );
              ...
              {sortedEvents.map((event) => (
                <div key={event._id} className="flex gap-5 items-start">
                  <div className="flex flex-col items-center">
                    <div className="w-3 h-3 rounded-full" style={{ background: '#b8956a' }} />
                    <div className="w-px h-full mt-2" style={{ background: 'rgba(180, 150, 100, 0.15)' }} />
                  </div>
                  <div className="flex-1 p-5 rounded-xl border" style={{ background: 'rgba(30, 25, 22, 0.4)', borderColor: 'rgba(180, 150, 100, 0.15)' }}>
                    ...
                    <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm" style={{ color: 'rgba(180, 150, 100, 0.7)' }}>
                      ...
                      <div>{event.acceptedCount ?? 0}/{event.totalPositions ?? 0} hired</div>
                      <div>{event.filled ? 'Filled' : `${event.totalPositions - (event.acceptedCount ?? 0)} open`}</div>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="card max-w-2xl w-full max-h-[80vh] overflow-hidden animate-scale-in">
            <div className="p-6 border-b flex justify-between items-center" style={{ borderColor: 'rgba(180, 150, 100, 0.15)' }}>
              ...
              <button
                ...
                style={{ color: 'rgba(180, 150, 100, 0.6)' }}
                onMouseEnter={(e) => { e.target.style.background = 'rgba(180, 150, 100, 0.1)'; }}
