...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-10">
      <h1 className="text-3xl md:text-4xl font-bold text-[color:var(--text)] mb-8 animate-slide-up">
        ...
        <div className="w-full lg:w-1/3 space-y-6">
          <div className="glass-card p-8 text-center flex flex-col items-center animate-scale-in">
            ...
            <p className="text-[color:var(--text-muted)] mb-3">
              {user?.role === "organizer"
                ? t("common.event_organizer", "Event Organizer")
                : t("common.hospitality_worker", "Hospitality Worker")}
            </p>
            <div className="flex flex-col items-center gap-1 mb-6">
              ...
              <span className="text-sm text-[color:var(--text-dim)]">
                {reviews.length} {t("common.reviews", "reviews")}
              </span>
            </div>
        ...
        <div className="w-full lg:w-2/3 space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[color:var(--text-dim)]">
                      {t("common.in_hospitality", "in Hospitality")}
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            <h2 className="text-xl font-semibold text-[color:var(--text)] mb-6">
              {t("common.recent_reviews", "Recent Reviews")}
            </h2>
            {reviews.length > 0 ? (
              <div className="space-y-4 max-h-96 overflow-y-auto pr-2">
                {reviews.map((review) => (
                  <div
                    key={review._id}
                    className="p-4 rounded-xl border border-[color:var(--border)] bg-[color:var(--surface-raised)]"
                  >
                    ...
                    <p className="text-[color:var(--text-muted)] text-sm mb-2">
                      {review.comment || t("common.no_comment", "No comment")}
                    </p>
              ...
              <p className="text-[color:var(--text-dim)] text-center py-6">
            ...
            >
              <div className="p-6 space-y-4 overflow-y-auto">
                <div>
                  <label className="block text-sm font-medium text-[color:var(--text-muted)] mb-1">
                    {t("common.avatar_image", "Avatar Image")}
                  </label>
                  <input
                    type="file"
                    accept="image/*"
                    onChange={(e) => setAvatarFile(e.target.files[0])}
                    className="w-full px-4 py-2.5 rounded-xl glass-input text-[color:var(--text)]"
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-[color:var(--text-muted)] mb-1">
                    {t("common.contact_number", "Contact Number")}
                  </label>
                  <input
                    type="tel"
                    name="contactNumber"
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${
                            formData.skills.includes(skill)
                              ? "bg-[color:var(--accent)] text-[color:var(--text-primary)]"
              ...
              <div className="p-4 bg-[color:var(--bg)] border-t border-[color:var(--border)] flex gap-3 sticky bottom-0 shrink-0">
                <button
                  type="button"
                  onClick={() => setEditing(false)}
                  className="flex-1 py-2.5 rounded-xl font-medium text-[color:var(--text-muted)] hover:bg-[color:var(--surface-raised)] transition-colors"
                >
